...
def process_task(task: DownloadTask, session):
    print(f"{Colors.WORKER} Processing task: {task.id} - {task.url}")
    ...
    try:
        ...
        print(f"{Colors.WORKER} Downloaded: {file_path}")

        # Step 2: Transcribe audio
        transcript = transcribe_audio(file_path)
        # print(f"{Colors.WORKER} Transcription complete.")

        # Step 3: Generate recipe JSON using LLM
        description = metadata["description"]
        source_url = task.url
        recipe_json = generate_recipe_json(description, transcript, source_url)
        print(f"{Colors.WORKER} Recipe JSON generated.")
        ...
        print(f"{Colors.WORKER} Recipe pushed to Tandoor.")
        ...
        print(f"{Colors.WORKER} Task {task.id} failed: {type(e).__name__}: {e}")
        task.status = TaskStatus.FAILED
        session.commit()

def worker_loop(poll_interval=5):
    print(f"{Colors.WORKER} Starting worker loop...")
    while True:
        ...
        if task:
            ...
            session.close()
            print(f"{Colors.WORKER} No pending tasks. Sleeping...")
